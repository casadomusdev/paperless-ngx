...
export class SavedViewService extends AbstractPaperlessService<SavedView> {
  ...
  // RKC: Convert to signal so computed signals in dashboard component react to changes
  ...
  public list(
    ...
    return super.list(page, pageSize, sortField, sortReverse, extraParams).pipe(
      tap({
        next: (r) => {
          // RKC: Update signal instead of plain array to trigger reactivity
  ...
  // RKC: Convert to computed signals for reactivity
