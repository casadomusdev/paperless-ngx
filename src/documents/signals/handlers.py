...
# RKC: Auto-create UiSettings for new users to prevent SSO signup errors
...
def create_ui_settings_for_new_user(sender, instance: User, created: bool, **kwargs):
    if created:
        try:
            ...
            if settings.DEBUG_SSO:
                logger.debug(f"UiSettings {'created' if was_created else 'already existed'} for user: {instance.username}")
        except Exception as e:
            # CRITICAL: Always log actual errors at ERROR level, but don't raise
            logger.exception(f"ERROR creating UiSettings for {instance.username}: {e}")
            logger.error("Continuing despite UiSettings creation failure - user signup will proceed")
# /end RKC edit
    ...
    def email_action():
        ...
        if settings.MAIL_VERIFY_RECIPIENT != "none":
            ...
            if verification_failures:
                ...
                logger.error(
                    f"Recipient verification failed for document '{title}': {failure_summary}. Email not sent.",
                    extra={"group": logging_group},
                )
                if not use_overrides and isinstance(document, Document):
                    # Apply action-level error tag
                    ...
                    if settings.MAIL_SEND_FAILURE_TAG_ID is not None:
                        ...
                        logger.info(
                            f"Recipient verify failure: queued add of tag {settings.MAIL_SEND_FAILURE_TAG_ID} to '{title}'",
                            extra={"group": logging_group},
                        )
                    if settings.MAIL_SEND_SUCCESS_TAG_ID is not None:
                        ...
                        logger.info(
                            f"Recipient verify failure: queued remove of tag {settings.MAIL_SEND_SUCCESS_TAG_ID} from '{title}'",
                            extra={"group": logging_group},
                        )
