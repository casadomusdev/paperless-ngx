...
- **v1.5.2** — Select all across pages for management lists: Gmail-style "Select all N items" link in the footer of Correspondents, Tags, Document Types, and Storage Paths lists. Uses the existing `all` field from the API response (zero additional API calls). When items are selected on the current page, a link appears to select all matching items across all pages. When all are selected, shows "All N selected" with a clear selection link.
